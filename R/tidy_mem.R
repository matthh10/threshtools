...
#' @param ecdc_format A boolean operator (default = `FALSE`). If `TRUE` uses
#'  format required for annual submission to ECDC which includes post-epidemic values.
#'
...
tidy_mem <- function(mem_model, decimal_places = 2, ecdc_format = FALSE){

  if (ecdc_format == TRUE) {

    mem_thresholds <-
      tibble::tibble(
        threshold = c("pre_epidemic", "post_epidemic", "CI40", "CI90", "C975"),
        value =
          janitor::round_half_up(
            c(mem_model$epidemic.thresholds[1], # pre-epidemic threshold (i.e. Low)
              mem_model$epidemic.thresholds[2], # post-epidemic threshold
